...
public class CheckParamsUtil {
    ...
    private static final Log                    log            = LogFactory.getLog (CheckParamsUtil.class);
...
//                    }
    ...
    private static boolean warnValue(String value){
        ...
        for ( int i = 0 ; i < warnStr.length ; i++ ) {
            if (value.indexOf (warnStr[i]) > -1) { return true; }
        }
        return false;
    }
    ...
    private static void checkParam(String key,String value) throws Exception{
        if (StringUtil.isEmpty (value)) { throw new Exception (key + " param must not be empty"); }
    }
